...
setup_repo() {
  ...
  (
    cd "$repo"
    git init >/dev/null
    mkdir -p docs/plans docs/reports docs/runbooks docs/specs heuristic-system/error-inbox
...
EOF_INNER
    cat > heuristic-system/error-inbox/a-gap.md <<'EOF_INNER'
# A gap
...
echo "$dry_run_output" | grep -Fq -- "- heuristic-system/error-inbox/a-gap.md"
...
[[ -f "${repo_one}/heuristic-system/error-inbox/a-gap.md" ]]
...
[[ -f "${repo_two}/heuristic-system/error-inbox/a-gap.md" ]]
