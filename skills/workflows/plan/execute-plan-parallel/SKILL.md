...
- Prefer using repo tooling (avoid parsing drift):
  - Lint: `$CODEX_HOME/scripts/plan/validate_plans.sh --file <plan.md>`
  - Parse JSON: `$CODEX_HOME/scripts/plan/plan_to_json.sh --file <plan.md> [--sprint <n>]`
  - Compute batches: `$CODEX_HOME/scripts/plan/plan_batches.sh --file <plan.md> --sprint <n>`
...
- Batch-launch subagents for tasks that have no unmet dependencies (use `$CODEX_HOME/scripts/plan/plan_batches.sh` output when available).
