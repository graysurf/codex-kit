...
```bash
$AGENTS_HOME/skills/tools/devex/desktop-notify/scripts/project-notify.sh "your short message" --level success
```

If you need a custom title, call the notifier directly:

```bash
$AGENTS_HOME/skills/tools/devex/desktop-notify/scripts/desktop-notify.sh \
  --title "custom title" \
...
- `PROJECT_PATH`: used by `$AGENTS_HOME/skills/tools/devex/desktop-notify/scripts/project-notify.sh` to derive the project title (fallback: git root, then `$PWD`)
