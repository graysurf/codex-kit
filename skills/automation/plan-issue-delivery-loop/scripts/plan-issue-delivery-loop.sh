...
for idx in table_rows[2:]:
    ...
    if spec is not None:
        ...
        if grouping_mode == "per-sprint":
            ...
            mode = "pr-shared" if group_sizes.get(pr_group, 0) > 1 else "pr-isolated"
            execution_source = group_anchor.get(pr_group, spec)
        else:
            raise SystemExit(
                f"error: unsupported pr-grouping in task spec for {task_id}: {grouping_mode or '<empty>'}"
...
enforce_sprint_merge_gate() {
  local issue_number="${1:-}"
  local sprint="${2:-}"
  local repo_arg="${3:-}"
  local dry_run="${4:-0}"
  local mark_done="${5:-0}" # 0=check only, 1=check and mark sprint task rows done

  [[ -n "$issue_number" ]] || die "issue number is required for sprint merge gate"
  [[ -n "$sprint" ]] || die "sprint number is required for sprint merge gate"
  is_positive_int "$sprint" || die "sprint must be a positive integer for sprint merge gate"

  if [[ "$dry_run" == '1' ]]; then
    printf 'SPRINT_MERGE_GATE=SKIP_DRY_RUN\n'
    printf 'SPRINT=%s\n' "$sprint"
    printf 'SPRINT_STATUS_MARK_DONE=%s\n' "$mark_done"
    return 0
  fi
  ...
  set +e
  local gate_output=''
  gate_output="$(python3 - "$issue_body_file" "$sprint" "$repo_arg" "$mark_done" "$updated_body_file" <<'PY'
import json
import pathlib
import re
import subprocess
...
body_path = pathlib.Path(sys.argv[1])
sprint = sys.argv[2].strip()
repo_arg = sys.argv[3].strip()
mark_done = sys.argv[4].strip() == "1"
out_path = pathlib.Path(sys.argv[5])

if not body_path.is_file():
    raise SystemExit(f"error: issue body file not found: {body_path}")
if not sprint.isdigit() or int(sprint) <= 0:
    raise SystemExit(f"error: invalid sprint number: {sprint}")
...
required_cols = ["Task", "PR", "Status", "Notes"]
missing = [col for col in required_cols if col not in headers]
if missing:
    raise SystemExit("error: missing Task Decomposition columns: " + ", ".join(missing))

idx_task = headers.index("Task")
idx_pr = headers.index("PR")
idx_status = headers.index("Status")
idx_notes = headers.index("Notes")

sprint_rows: list[tuple[int, list[str]]] = []
target_sprint_tag = f"S{sprint}"
task_pat = re.compile(rf"^S{sprint}T\d+$", flags=re.IGNORECASE)

for line_idx in table_rows[2:]:
    cells = parse_row(lines[line_idx])
    if not cells or len(cells) != len(headers):
        continue
    task_id = cells[idx_task].strip()
    notes = cells[idx_notes].strip()
    note_sprint = extract_note_value(notes, "sprint").upper()
    in_sprint = note_sprint == target_sprint_tag or bool(task_pat.fullmatch(task_id))
    if in_sprint:
        sprint_rows.append((line_idx, cells))

if not sprint_rows:
    raise SystemExit(f"error: no Task Decomposition rows found for sprint {sprint}")

errors: list[str] = []
pr_to_tasks: dict[str, list[str]] = {}

for _line_idx, cells in sprint_rows:
    task_id = cells[idx_task].strip() or "<unknown-task>"
    status = cells[idx_status].strip()
    pr_value = cells[idx_pr].strip()

    if not mark_done and status.strip().lower() != "done":
        errors.append(f"{task_id}: Status must be done before next sprint (got: {status or '<empty>'})")

    pr_number = normalize_pr_number(pr_value)
    if not pr_number:
        errors.append(f"{task_id}: PR must be a concrete merged PR reference before sprint close gate (got: {pr_value or '<empty>'})")
        continue
    pr_to_tasks.setdefault(pr_number, []).append(task_id)

for pr_number, tasks in sorted(pr_to_tasks.items(), key=lambda item: int(item[0])):
    cmd = ["gh", "pr", "view", pr_number, "--json", "number,state,mergedAt,url"]
    if repo_arg:
        cmd.extend(["-R", repo_arg])
    proc = subprocess.run(cmd, capture_output=True, text=True)
    if proc.returncode != 0:
        detail = (proc.stderr or proc.stdout or "").strip() or f"exit {proc.returncode}"
        errors.append(f"tasks [{', '.join(tasks)}]: failed to query PR #{pr_number}: {detail}")
        continue
    data = json.loads(proc.stdout or "{}")
    merged_at = (data.get("mergedAt") or "").strip()
    if not merged_at:
        pr_url = data.get("url") or f"#{pr_number}"
        pr_state = data.get("state") or "UNKNOWN"
        errors.append(f"tasks [{', '.join(tasks)}]: PR is not merged ({pr_url}, state={pr_state})")
...
updated_rows = 0
if mark_done:
    for line_idx, cells in sprint_rows:
        if cells[idx_status].strip().lower() != "done":
            cells[idx_status] = "done"
            lines[line_idx] = "| " + " | ".join(cells) + " |"
            updated_rows += 1

out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
print(f"SPRINT={sprint}")
print(f"SPRINT_TASK_COUNT={len(sprint_rows)}")
print(f"SPRINT_PR_COUNT={len(pr_to_tasks)}")
print(f"SPRINT_STATUS_ROWS_MARKED_DONE={updated_rows}")
print("SPRINT_MERGE_GATE=PASS")
PY
)"
  local gate_rc=$?
  set -e
  ...
  if [[ "$gate_rc" -ne 0 ]]; then
    printf '%s\n' "$gate_output" >&2
    rm -f "$issue_body_file" "$updated_body_file"
    return "$gate_rc"
  fi
  ...
  if [[ "$mark_done" == '1' ]]; then
    run_issue_lifecycle "$dry_run" "$repo_arg" update --issue "$issue_number" --body-file "$updated_body_file" >/dev/null
    printf 'SPRINT_STATUS_SYNC=UPDATED_TO_DONE\n'
  fi
...
if mode == "start":
    ...
    lead = "Main-agent requests sprint-level review before merge/acceptance on the plan issue (the issue remains open)."
else:
    heading = f"## Sprint {sprint} Accepted"
    lead = "Main-agent records sprint acceptance after merge gate passes and sprint rows are synced to done (issue remains open for remaining sprints)."
...
start_sprint_cmd() {
  ...
  if (( sprint > 1 )); then
    ...
    enforce_sprint_merge_gate "$issue_number" "$previous_sprint" "$repo_arg" "$dry_run" '0'
    printf 'PREVIOUS_SPRINT_GATE=PASS sprint=%s\n' "$previous_sprint"
  fi
  ...
  if [[ "$post_comment" == '1' ]]; then
    ...
    # Avoid failing after comment post; retries would duplicate comments.
...
accept_sprint_cmd() {
  ...
  render_task_spec_from_plan_scope "$plan_file" sprint "$sprint" "$task_spec_out" "$owner_prefix" "$branch_prefix" "$worktree_prefix" "$pr_grouping" "$pr_group_config" >/dev/null
  sync_issue_sprint_task_rows "$issue_number" "$task_spec_out" "$repo_arg" "$dry_run"
  enforce_sprint_merge_gate "$issue_number" "$sprint" "$repo_arg" "$dry_run" '1'
...
multi_sprint_guide_cmd() {
  ...
  printf 'NOTE_SPRINT_GATE=%s\n' "Before starting sprint N+1, sprint N must be reviewed, merged, and accepted."
  printf 'NOTE_ACCEPT_SYNC=%s\n' "accept-sprint enforces merged PRs for the sprint and syncs sprint task Status to done."
