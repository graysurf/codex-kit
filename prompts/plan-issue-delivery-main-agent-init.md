...
3) For each sprint: `start-sprint` -> verify `TASK_PROMPT_PATH` + `PLAN_SNAPSHOT_PATH` artifacts under `$AGENT_HOME/out/plan-issue-delivery/...` -> delegate to subagents -> `ready-sprint` -> review approval -> merge PRs -> `accept-sprint`.
...
- Always include `$AGENT_HOME/prompts/plan-issue-delivery-subagent-init.md` as a required companion prompt.
